...
// minimalValidOrder returns the minimal PlaceOrderRequest that passes
// all non-ClientOrderId validations so tests can focus on the cloid path.
...
func Test_ValidatePlaceOrdersAction_RejectsReservedInternalClientOrderIdPrefix(t *testing.T) {
	...
	order := minimalValidOrder()
	order.ClientOrderId = snx_lib_api_json.ClientOrderId("snx-internal/chunk-123")
...
// ---------------------------------------------------------------------------
// Price accepts fractional values at this layer
// ---------------------------------------------------------------------------
