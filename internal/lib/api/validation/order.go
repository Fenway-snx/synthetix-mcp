...
// Maximum allowed length for the source field.
...
// Payload-only shape without auth-related fields.
...
// Applies order ID validation not triggered by mapstructure decoding.
...
// Validates a source string, treating empty input as unspecified.
func ValidateSource(source string) (string, error) {
	...
	// Empty after trimming remains unspecified.
...
func ValidateCancelOrdersAction(action *CancelOrdersActionPayload) ([]VenueOrderId, error) {
	...
	// Keep explicit validation here to satisfy existing unit tests.

	sanitised := make([]VenueOrderId, 0, len(action.VenueOrderIds))
	for i := range action.VenueOrderIds {
		orderID, err := validateVenueOrderId(action.VenueOrderIds[i])
		if err != nil {
			return nil, err
		}
		sanitised = append(sanitised, orderID)
	}
...
// Validates modification fields with an optional deferred error prefix.
...
// Validates batch modifications using the same per-order rules.
// TODO: remove this if batch modification support is not intended.
