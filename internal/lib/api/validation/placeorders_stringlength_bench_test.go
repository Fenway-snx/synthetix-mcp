...
// benchPlaceOrdersFullBatch returns MaxOrdersPerBatch valid orders matching
// the batch size used by ValidatePlaceOrdersAction.
...
// Benchmark_ValidatePlaceOrdersAction_orderLoopStringMaxLength measures the
// per-order string max-length checks used inside ValidatePlaceOrdersAction.
// DYNAMIC_SPRINTF: legacy eager fmt.Sprintf (allocates field names on every
// call). DEFERRED_FUNC_THUNK: func() string thunks (matches production
// after the lazy fieldName change). STATIC_FIELD_NAMES: fixed labels,
// no Sprintf.
