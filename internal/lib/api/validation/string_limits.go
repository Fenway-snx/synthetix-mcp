...
// deferredValidationLabel is either a static label or a thunk that builds
// the label only when validation fails (avoiding eager fmt.Sprintf). Used
// by [ValidateCanonicalCollateralSymbol], [ValidateCanonicalSymbol],
// [ValidateEnum], [ValidateStringMaxLength], and validateModifyOrderFields.
...
// Rejects overlong string inputs before deeper parsing. Pass context as a
// string for static labels, or as func() string to defer building the label
// until an overlong value is detected.
//
// Note:
// The ability to pass a function is to defer the cost of creating the
// qualifying argument for the non-normative case; the prior implementation
// paid the cost on every call. See SNX-6054 for more detailed explanation
// and before/after benchmark results.
...
// Rejects symbols that are valid only after trimming or uppercasing. Pass
// context as the string "symbol" to return normalization errors unchanged,
// or pass a custom label as a string or func() string (evaluated only when
