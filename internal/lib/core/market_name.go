...
// Marshal the string value of the market name.
//
// Note:
// Unlike `MarketName#UnmarshalJSON()`, which applies correctness rules to
// an incoming market name, this function will marshal out, in string form,
// any `MarketName` instance, even when such an instance violates the naming
...
// Attempt to unmarshal a market name from a string representation.
//
// Note:
// Unlike `MarketName#MarshalJSON()`, which marshals any market name (even
