...
// CodedError is an error that carries a structured error code from its origin.
// This eliminates the need for string-based error-to-code mapping.
...
// ErrorCodeFrom extracts the error code from a CodedError in the chain.
// If the error is nil or no CodedError is found, returns the optional fallback
// (or empty string if no fallback provided).
...
var (
	...
	ErrTradingServiceDraining   = errors.New("trading service is draining, not accepting new orders")
	ErrTradingServiceIsNotReady = errors.New("trading service is not ready, still recovering :(")
	...
	ErrInvalidModifyOrderPayload = errors.New("please provide at least one of the following: price, quantity, trigger price") // price is not used for market orders though
