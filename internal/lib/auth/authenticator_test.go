...
func Test_Authenticator_ValidateAuthentication_NonceStoreError(t *testing.T) {
	...
	nonceStore.On("ReserveNonce", testAddress.Hex(), Nonce(timestamp)).Return(false, errors.New("nonce store connection failed"))
	...
	assert.Contains(t, err.Error(), "nonce store connection failed")
...
func Test_Authenticator_ValidateAuthentication_ReserveNonceError(t *testing.T) {
	...
	nonceStore.On("ReserveNonce", testAddress.Hex(), Nonce(timestamp)).Return(false, errors.New("nonce store write error"))
	...
	assert.Nil(t, result)
	assert.Contains(t, err.Error(), "failed to reserve timestamp")
	assert.Contains(t, err.Error(), "nonce store write error")
...
func Test_Authenticator_VerifyAccountOwnership_CacheHit(t *testing.T) {
	...
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.True(t, owns)
	assert.Equal(t, 1, mockClient.VerifyCallCount, "verifier should not be called on cache hit")
}
...
func Test_Authenticator_VerifyAccountOwnership_NegativelyCachesRefusal(t *testing.T) {
	...
	assert.False(t, owns)
	assert.Error(t, err)
	assert.Equal(t, 1, mockClient.VerifyCallCount, "verifier should not be called on negative cache hit")
}
...
func Test_Authenticator_VerifyAccountOwner_CacheHit_Delegate_Rejected(t *testing.T) {
	...
	// VerifyAccountOwner should reject delegates from cache without a verifier call.
	isOwner, err := authenticator.VerifyAccountOwner(testAddress, testSubAccountID)
	assert.False(t, isOwner)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "delegate but not the owner")
	assert.Equal(t, 0, mockClient.VerifyCallCount, "verifier should not be called when cache resolves delegate")
}
...
func Test_Authenticator_VerifyAccountOwner_DelegateFromVerifier_NotCachedAsOwner(t *testing.T) {
	cache := NewAuthCache(100)
	mockClient := newFakeSubaccountVerifier()
	...
	// The verifier returns authorized=true but authType=DELEGATE, so VerifyAccountOwner should reject.
	isOwner, err := authenticator.VerifyAccountOwner(testAddress, testSubAccountID)
	assert.False(t, isOwner)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "delegate but not the owner")
...
func Test_Authenticator_EvictAuth_RemovesFromCache(t *testing.T) {
	...
	// Next call should hit the verifier again.
	_, err = authenticator.VerifyAccountOwnership(testAddress, testSubAccountID)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	assert.Equal(t, 2, mockClient.VerifyCallCount, "verifier should be called after eviction")
}
...
func Test_Authenticator_VerifyAccountOwnership_NilCache(t *testing.T) {
	...
	// Without cache, second call also hits the verifier.
	owns, err = authenticator.VerifyAccountOwnership(testAddress, testSubAccountID)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	assert.True(t, owns)
	assert.Equal(t, 2, mockClient.VerifyCallCount, "without cache, every call should hit the verifier")
}
...
func Test_Authenticator_VerifyAccountOwnership_CacheHit_DelegateAlsoAccepted(t *testing.T) {
	...
	// Prime cache with delegate via verifier call.
	owns, err := authenticator.VerifyAccountOwnership(testAddress, testSubAccountID)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.True(t, owns)
	assert.Equal(t, 1, mockClient.VerifyCallCount)
...
func Test_Authenticator_VerifyAccountOwnership_NegativeCacheExpires(t *testing.T) {
	...
	assert.False(t, owns)
	assert.Error(t, err)
	assert.Equal(t, 2, mockClient.VerifyCallCount, "verifier should be called after negative entry expires")
}
...
func Test_Authenticator_VerifyAccountOwnership_PositiveResultClearsNegativeCache(t *testing.T) {
	...
	// Grant access and expire the negative entry so the verifier is called again.
	mockClient.AddOwner(testAddress, snx_lib_core.SubAccountId(testAccountIDUint))
	cache.negativeTTL = 0
