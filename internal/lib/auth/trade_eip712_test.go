...
func Test_CreateTradeTypedData_CreateSubaccount(t *testing.T) {
	...
	// subAccountId is used as masterSubAccountId for ownership verification
...
// --- Voluntary Auto-Exchange tests ---

func Test_GetVoluntaryAutoExchangeTypes(t *testing.T) {
	types := GetVoluntaryAutoExchangeTypes()

	// Should have EIP712Domain and VoluntaryAutoExchange entries
	require.Contains(t, types, "EIP712Domain")
	require.Contains(t, types, "VoluntaryAutoExchange")

	fields := types["VoluntaryAutoExchange"]
	require.Len(t, fields, 5)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "uint256", fields[0].Type)
	require.Equal(t, "sourceAsset", fields[1].Name)
	require.Equal(t, "string", fields[1].Type)
	require.Equal(t, "targetUSDTAmount", fields[2].Name)
	require.Equal(t, "string", fields[2].Type)
	require.Equal(t, "nonce", fields[3].Name)
	require.Equal(t, "uint256", fields[3].Type)
	require.Equal(t, "expiresAfter", fields[4].Name)
...
func Test_ensureValidatedVoluntaryAutoExchange_Pointer(t *testing.T) {
	payload := &snx_lib_api_validation.VoluntaryAutoExchangeActionPayload{
		Action:           "voluntaryAutoExchange",
		SourceAsset:      "ETH",
		TargetUSDTAmount: "100.5",
	}

	result, err := ensureValidatedVoluntaryAutoExchange(payload)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "ETH", result.SourceAsset)
	require.Equal(t, "100.5", result.TargetUSDTAmount)
}

func Test_ensureValidatedVoluntaryAutoExchange_Value(t *testing.T) {
	payload := snx_lib_api_validation.VoluntaryAutoExchangeActionPayload{
		Action:           "voluntaryAutoExchange",
		SourceAsset:      "BTC",
	...
	result, err := ensureValidatedVoluntaryAutoExchange(payload)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "BTC", result.SourceAsset)
	require.Equal(t, "all", result.TargetUSDTAmount)
}

func Test_ensureValidatedVoluntaryAutoExchange_ValidatedPointer(t *testing.T) {
	validated := &snx_lib_api_validation.ValidatedVoluntaryAutoExchangeAction{
		Payload: &snx_lib_api_validation.VoluntaryAutoExchangeActionPayload{
			Action:           "voluntaryAutoExchange",
			SourceAsset:      "ETH",
			TargetUSDTAmount: "50",
	...
	result, err := ensureValidatedVoluntaryAutoExchange(validated)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, validated, result)
}

func Test_ensureValidatedVoluntaryAutoExchange_ValidatedValue(t *testing.T) {
	validated := snx_lib_api_validation.ValidatedVoluntaryAutoExchangeAction{
		Payload: &snx_lib_api_validation.VoluntaryAutoExchangeActionPayload{
			Action:           "voluntaryAutoExchange",
			SourceAsset:      "ETH",
	...
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "ETH", result.SourceAsset)
	require.Equal(t, "50", result.TargetUSDTAmount)
}

func Test_ensureValidatedVoluntaryAutoExchange_Map(t *testing.T) {
	payload := map[string]any{
		"action":           "voluntaryAutoExchange",
		"sourceAsset":      "ETH",
		"targetUSDTAmount": "200",
	}

	result, err := ensureValidatedVoluntaryAutoExchange(payload)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "ETH", result.SourceAsset)
	require.Equal(t, "200", result.TargetUSDTAmount)
}

func Test_ensureValidatedVoluntaryAutoExchange_UnsupportedType(t *testing.T) {
	_, err := ensureValidatedVoluntaryAutoExchange("invalid")
	require.Error(t, err)
	require.Contains(t, err.Error(), "unsupported voluntaryAutoExchange payload type")
}

func Test_ensureValidatedVoluntaryAutoExchange_MapDecodeFailure(t *testing.T) {
	payload := map[string]any{
		"action":           []int{1, 2, 3}, // slice cannot be decoded to string
		"sourceAsset":      "ETH",
	...
	_, err := ensureValidatedVoluntaryAutoExchange(payload)
	require.Error(t, err)
}

func Test_CreateTradeTypedData_VoluntaryAutoExchange(t *testing.T) {
	payload := &snx_lib_api_validation.VoluntaryAutoExchangeActionPayload{
		Action:           "voluntaryAutoExchange",
		SourceAsset:      "ETH",
	...
	typed, err := CreateTradeTypedData("123", 42, 1234567890, "voluntaryAutoExchange", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "VoluntaryAutoExchange", typed.PrimaryType)

	subAccountId, ok := typed.Message["subAccountId"].(string)
	require.True(t, ok)
	require.Equal(t, "123", subAccountId)

	sourceAsset, ok := typed.Message["sourceAsset"].(string)
	require.True(t, ok)
	require.Equal(t, "ETH", sourceAsset)

	targetUSDTAmount, ok := typed.Message["targetUSDTAmount"].(string)
	require.True(t, ok)
	require.Equal(t, "500.25", targetUSDTAmount)
	...
	// Verify the types match GetVoluntaryAutoExchangeTypes
	fields := typed.Types["VoluntaryAutoExchange"]
	require.Len(t, fields, 5)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "sourceAsset", fields[1].Name)
	require.Equal(t, "targetUSDTAmount", fields[2].Name)
...
func Test_CreateTradeTypedData_VoluntaryAutoExchange_All(t *testing.T) {
	payload := &snx_lib_api_validation.VoluntaryAutoExchangeActionPayload{
		Action:           "voluntaryAutoExchange",
		SourceAsset:      "BTC",
		TargetUSDTAmount: "all",
	...
	typed, err := CreateTradeTypedData("456", 1, 9999, "voluntaryAutoExchange", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "VoluntaryAutoExchange", typed.PrimaryType)
	require.Equal(t, "BTC", typed.Message["sourceAsset"])
	require.Equal(t, "all", typed.Message["targetUSDTAmount"])

	_, err = GetEIP712MessageHash(typed)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
}

func Test_CreateTradeTypedData_VoluntaryAutoExchange_InvalidPayload(t *testing.T) {
	_, err := CreateTradeTypedData("123", 42, 1234567890, "voluntaryAutoExchange", "invalid", DefaultDomainName, "1", 1)
	require.Error(t, err)
	require.Contains(t, err.Error(), "voluntaryAutoExchange payload")
}

func Test_CreateTradeTypedData_VoluntaryAutoExchange_FromMap(t *testing.T) {
	payload := map[string]any{
		"action":           "voluntaryAutoExchange",
		"sourceAsset":      "SOL",
		"targetUSDTAmount": "1000",
	}

	typed, err := CreateTradeTypedData("789", 10, 9999999, "voluntaryAutoExchange", payload, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "VoluntaryAutoExchange", typed.PrimaryType)
	require.Equal(t, "SOL", typed.Message["sourceAsset"])
	require.Equal(t, "1000", typed.Message["targetUSDTAmount"])

	_, err = GetEIP712MessageHash(typed)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
}

// --- Snaxpot EIP-712 tests ---

func Test_GetClearSnaxpotPreferenceTypes(t *testing.T) {
	types := GetClearSnaxpotPreferenceTypes()

	require.Contains(t, types, "EIP712Domain")
	require.Contains(t, types, "ClearSnaxpotPreference")

	fields := types["ClearSnaxpotPreference"]
	require.Len(t, fields, 4)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "uint256", fields[0].Type)
	require.Equal(t, "scope", fields[1].Name)
	require.Equal(t, "string", fields[1].Type)
	require.Equal(t, "nonce", fields[2].Name)
	require.Equal(t, "uint256", fields[2].Type)
	require.Equal(t, "expiresAfter", fields[3].Name)
...
func Test_GetSaveSnaxpotTicketsTypes(t *testing.T) {
	types := GetSaveSnaxpotTicketsTypes()

	require.Contains(t, types, "EIP712Domain")
	require.Contains(t, types, "SnaxpotTicket")
	require.Contains(t, types, "SaveSnaxpotTickets")

	ticketFields := types["SnaxpotTicket"]
	require.Len(t, ticketFields, 7)

	fields := types["SaveSnaxpotTickets"]
	require.Len(t, fields, 4)
	require.Equal(t, "entries", fields[1].Name)
	require.Equal(t, "SnaxpotTicket[]", fields[1].Type)
}

func Test_GetSetSnaxpotPreferenceTypes(t *testing.T) {
	types := GetSetSnaxpotPreferenceTypes()

	require.Contains(t, types, "EIP712Domain")
	require.Contains(t, types, "SetSnaxpotPreference")

	fields := types["SetSnaxpotPreference"]
	require.Len(t, fields, 5)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "uint256", fields[0].Type)
	require.Equal(t, "snaxBall", fields[1].Name)
	require.Equal(t, "uint256", fields[1].Type)
	require.Equal(t, "scope", fields[2].Name)
	require.Equal(t, "string", fields[2].Type)
	require.Equal(t, "nonce", fields[3].Name)
	require.Equal(t, "uint256", fields[3].Type)
	require.Equal(t, "expiresAfter", fields[4].Name)
...
func Test_CreateTradeTypedData_SetSnaxpotPreference(t *testing.T) {
	payload := &snx_lib_api_validation.SetSnaxpotPreferenceActionPayload{
		Action:   "setSnaxpotPreference",
		Scope:    "persistent",
		SnaxBall: 3,
	}
	validated := &snx_lib_api_validation.ValidatedSetSnaxpotPreferenceAction{Payload: payload}

	typed, err := CreateTradeTypedData("123", 42, 1234567890, "setSnaxpotPreference", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "SetSnaxpotPreference", typed.PrimaryType)

	subAccountId, ok := typed.Message["subAccountId"].(string)
	require.True(t, ok)
	require.Equal(t, "123", subAccountId)

	snaxBall, ok := typed.Message["snaxBall"].(string)
	require.True(t, ok)
	require.Equal(t, "3", snaxBall)

	scope, ok := typed.Message["scope"].(string)
	require.True(t, ok)
	require.Equal(t, "persistent", scope)
	...
	fields := typed.Types["SetSnaxpotPreference"]
	require.Len(t, fields, 5)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "uint256", fields[0].Type)
	require.Equal(t, "snaxBall", fields[1].Name)
...
func Test_CreateTradeTypedData_SetSnaxpotPreference_CurrentEpoch(t *testing.T) {
	payload := &snx_lib_api_validation.SetSnaxpotPreferenceActionPayload{
		Action:   "setSnaxpotPreference",
		Scope:    "currentEpoch",
		SnaxBall: 1,
	...
	typed, err := CreateTradeTypedData("7", 10, 0, "setSnaxpotPreference", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "SetSnaxpotPreference", typed.PrimaryType)
	require.Equal(t, "1", typed.Message["snaxBall"])
	require.Equal(t, "currentEpoch", typed.Message["scope"])

	_, err = GetEIP712MessageHash(typed)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
}

func Test_CreateTradeTypedData_SetSnaxpotPreference_InvalidPayload(t *testing.T) {
	_, err := CreateTradeTypedData("123", 42, 1234567890, "setSnaxpotPreference", "invalid", DefaultDomainName, "1", 1)
	require.Error(t, err)
	require.Contains(t, err.Error(), "setSnaxpotPreference payload")
}

func Test_CreateTradeTypedData_SetSnaxpotPreference_FromMap(t *testing.T) {
	payload := map[string]any{
		"action":   "setSnaxpotPreference",
		"scope":    "persistent",
		"snaxBall": int32(5),
	...
	typed, err := CreateTradeTypedData("99", 7, 9999, "setSnaxpotPreference", payload, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "SetSnaxpotPreference", typed.PrimaryType)
	require.Equal(t, "5", typed.Message["snaxBall"])
	require.Equal(t, "persistent", typed.Message["scope"])

	_, err = GetEIP712MessageHash(typed)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
}

func Test_CreateTradeTypedData_ClearSnaxpotPreference(t *testing.T) {
	payload := &snx_lib_api_validation.ClearSnaxpotPreferenceActionPayload{
		Action: "clearSnaxpotPreference",
		Scope:  "persistent",
	}
	validated := &snx_lib_api_validation.ValidatedClearSnaxpotPreferenceAction{Payload: payload}

	typed, err := CreateTradeTypedData("456", 55, 9999999, "clearSnaxpotPreference", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "ClearSnaxpotPreference", typed.PrimaryType)
	...
	nonceStr, ok := typed.Message["nonce"].(string)
	require.True(t, ok)
	require.Equal(t, "55", nonceStr)

	expiresStr, ok := typed.Message["expiresAfter"].(string)
	require.True(t, ok)
	require.Equal(t, "9999999", expiresStr)

	fields := typed.Types["ClearSnaxpotPreference"]
	require.Len(t, fields, 4)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "uint256", fields[0].Type)
	require.Equal(t, "scope", fields[1].Name)
...
func Test_CreateTradeTypedData_ClearSnaxpotPreference_CurrentEpoch(t *testing.T) {
	payload := &snx_lib_api_validation.ClearSnaxpotPreferenceActionPayload{
		Action: "clearSnaxpotPreference",
		Scope:  "currentEpoch",
	}
	...
	typed, err := CreateTradeTypedData("1", 1, 1, "clearSnaxpotPreference", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "ClearSnaxpotPreference", typed.PrimaryType)
	require.Equal(t, "currentEpoch", typed.Message["scope"])
...
func Test_CreateTradeTypedData_ClearSnaxpotPreference_InvalidPayload(t *testing.T) {
	_, err := CreateTradeTypedData("456", 55, 9999999, "clearSnaxpotPreference", "invalid", DefaultDomainName, "1", 1)
	require.Error(t, err)
	require.Contains(t, err.Error(), "clearSnaxpotPreference payload")
}

func Test_CreateTradeTypedData_ClearSnaxpotPreference_FromMap(t *testing.T) {
	payload := map[string]any{
		"action": "clearSnaxpotPreference",
		"scope":  "persistent",
	}

	typed, err := CreateTradeTypedData("2", 3, 500, "clearSnaxpotPreference", payload, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "ClearSnaxpotPreference", typed.PrimaryType)
	require.Equal(t, "persistent", typed.Message["scope"])
...
func Test_CreateTradeTypedData_SaveSnaxpotTickets(t *testing.T) {
	payload := &snx_lib_api_validation.SaveSnaxpotTicketsActionPayload{
		...
		Entries: []snx_lib_api_validation.SnaxpotTicketMutationEntryPayload{
			{Ball1: 1, Ball2: 2, Ball3: 3, Ball4: 4, Ball5: 5, SnaxBall: 2, TicketSerial: "42"},
		},
	}
	validated := &snx_lib_api_validation.ValidatedSaveSnaxpotTicketsAction{Payload: payload}

	typed, err := CreateTradeTypedData("789", 99, 5000000, "saveSnaxpotTickets", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "SaveSnaxpotTickets", typed.PrimaryType)

	subAccountId, ok := typed.Message["subAccountId"].(string)
	require.True(t, ok)
	require.Equal(t, "789", subAccountId)

	entries, ok := typed.Message["entries"].([]map[string]any)
	require.True(t, ok)
	require.Len(t, entries, 1)

	entry := entries[0]
	require.Equal(t, "1", entry["ball1"])
	require.Equal(t, "2", entry["ball2"])
	require.Equal(t, "3", entry["ball3"])
	require.Equal(t, "4", entry["ball4"])
	require.Equal(t, "5", entry["ball5"])
	require.Equal(t, "2", entry["snaxBall"])
	require.Equal(t, "42", entry["ticketSerial"])

	nonceStr, ok := typed.Message["nonce"].(string)
	require.True(t, ok)
	require.Equal(t, "99", nonceStr)

	expiresStr, ok := typed.Message["expiresAfter"].(string)
	require.True(t, ok)
	require.Equal(t, "5000000", expiresStr)

	fields := typed.Types["SaveSnaxpotTickets"]
	require.Len(t, fields, 4)
	require.Equal(t, "subAccountId", fields[0].Name)
	require.Equal(t, "uint256", fields[0].Type)
	require.Equal(t, "entries", fields[1].Name)
	...
	ticketFields := typed.Types["SnaxpotTicket"]
	require.Len(t, ticketFields, 7)
	require.Equal(t, "ball1", ticketFields[0].Name)
	require.Equal(t, "uint256", ticketFields[0].Type)

	_, err = GetEIP712MessageHash(typed)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
}

func Test_CreateTradeTypedData_SaveSnaxpotTickets_MultipleEntries(t *testing.T) {
	payload := &snx_lib_api_validation.SaveSnaxpotTicketsActionPayload{
		Action: "saveSnaxpotTickets",
		Entries: []snx_lib_api_validation.SnaxpotTicketMutationEntryPayload{
			{Ball1: 1, Ball2: 2, Ball3: 3, Ball4: 4, Ball5: 5, SnaxBall: 1, TicketSerial: "10"},
			{Ball1: 6, Ball2: 7, Ball3: 8, Ball4: 9, Ball5: 10, SnaxBall: 5, TicketSerial: "99"},
		},
	}
	validated := &snx_lib_api_validation.ValidatedSaveSnaxpotTicketsAction{Payload: payload}

	typed, err := CreateTradeTypedData("1", 1, 1, "saveSnaxpotTickets", validated, DefaultDomainName, "1", 1)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
	require.Equal(t, "SaveSnaxpotTickets", typed.PrimaryType)
	...
	require.Equal(t, "10", entries[0]["ticketSerial"])
	require.Equal(t, "99", entries[1]["ticketSerial"])

	_, err = GetEIP712MessageHash(typed)
	require.NoError(t, err, "expected `err` to be `nil`, but it was '%[1]s' (%[1]T)", err)
}

func Test_CreateTradeTypedData_SaveSnaxpotTickets_InvalidPayload(t *testing.T) {
	_, err := CreateTradeTypedData("789", 99, 5000000, "saveSnaxpotTickets", "invalid", DefaultDomainName, "1", 1)
	require.Error(t, err)
	require.Contains(t, err.Error(), "saveSnaxpotTickets payload")
}
