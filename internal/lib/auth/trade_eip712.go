...
func ensureValidatedClearSnaxpotPreference(action any) (*snx_lib_api_validation.ValidatedClearSnaxpotPreferenceAction, error) {
	switch v := action.(type) {
	case *snx_lib_api_validation.ClearSnaxpotPreferenceActionPayload:
		return snx_lib_api_validation.NewValidatedClearSnaxpotPreferenceAction(v)
	case snx_lib_api_validation.ClearSnaxpotPreferenceActionPayload:
		return snx_lib_api_validation.NewValidatedClearSnaxpotPreferenceAction(&v)
	case *snx_lib_api_validation.ValidatedClearSnaxpotPreferenceAction:
		return v, nil
	case snx_lib_api_validation.ValidatedClearSnaxpotPreferenceAction:
		return &v, nil
	case map[string]any:
		decoded, err := snx_lib_api_validation.DecodeClearSnaxpotPreferenceAction(v)
		if err != nil {
			return nil, err
		}
		return snx_lib_api_validation.NewValidatedClearSnaxpotPreferenceAction(decoded)
	default:
		return nil, fmt.Errorf("unsupported clearSnaxpotPreference payload type %T", action)
	}
}

func ensureValidatedSaveSnaxpotTickets(action any) (*snx_lib_api_validation.ValidatedSaveSnaxpotTicketsAction, error) {
	switch v := action.(type) {
	case *snx_lib_api_validation.SaveSnaxpotTicketsActionPayload:
		return snx_lib_api_validation.NewValidatedSaveSnaxpotTicketsAction(v)
	case snx_lib_api_validation.SaveSnaxpotTicketsActionPayload:
		return snx_lib_api_validation.NewValidatedSaveSnaxpotTicketsAction(&v)
	case *snx_lib_api_validation.ValidatedSaveSnaxpotTicketsAction:
		return v, nil
	case snx_lib_api_validation.ValidatedSaveSnaxpotTicketsAction:
		return &v, nil
	case map[string]any:
		decoded, err := snx_lib_api_validation.DecodeSaveSnaxpotTicketsAction(v)
		if err != nil {
			return nil, err
		}
		return snx_lib_api_validation.NewValidatedSaveSnaxpotTicketsAction(decoded)
	default:
		return nil, fmt.Errorf("unsupported saveSnaxpotTickets payload type %T", action)
	}
}

func ensureValidatedSetSnaxpotPreference(action any) (*snx_lib_api_validation.ValidatedSetSnaxpotPreferenceAction, error) {
	switch v := action.(type) {
	case *snx_lib_api_validation.SetSnaxpotPreferenceActionPayload:
		return snx_lib_api_validation.NewValidatedSetSnaxpotPreferenceAction(v)
	case snx_lib_api_validation.SetSnaxpotPreferenceActionPayload:
		return snx_lib_api_validation.NewValidatedSetSnaxpotPreferenceAction(&v)
	case *snx_lib_api_validation.ValidatedSetSnaxpotPreferenceAction:
		return v, nil
	case snx_lib_api_validation.ValidatedSetSnaxpotPreferenceAction:
		return &v, nil
	case map[string]any:
		decoded, err := snx_lib_api_validation.DecodeSetSnaxpotPreferenceAction(v)
		if err != nil {
			return nil, err
		}
		return snx_lib_api_validation.NewValidatedSetSnaxpotPreferenceAction(decoded)
	default:
		return nil, fmt.Errorf("unsupported setSnaxpotPreference payload type %T", action)
	}
}

func ensureValidatedVoluntaryAutoExchange(action any) (*snx_lib_api_validation.ValidatedVoluntaryAutoExchangeAction, error) {
	switch v := action.(type) {
	case *snx_lib_api_validation.VoluntaryAutoExchangeActionPayload:
		return snx_lib_api_validation.NewValidatedVoluntaryAutoExchangeAction(v)
	case snx_lib_api_validation.VoluntaryAutoExchangeActionPayload:
		return snx_lib_api_validation.NewValidatedVoluntaryAutoExchangeAction(&v)
	case *snx_lib_api_validation.ValidatedVoluntaryAutoExchangeAction:
		return v, nil
	case snx_lib_api_validation.ValidatedVoluntaryAutoExchangeAction:
		return &v, nil
	case map[string]any:
		decoded, err := snx_lib_api_validation.DecodeVoluntaryAutoExchangeAction(v)
		if err != nil {
			return nil, err
		}
		return snx_lib_api_validation.NewValidatedVoluntaryAutoExchangeAction(decoded)
	default:
		return nil, fmt.Errorf("unsupported voluntaryAutoExchange payload type %T", action)
	}
}
...
func toSaveSnaxpotTicketsTypedEntries(
	entries []snx_lib_api_validation.SnaxpotTicketMutationEntryPayload,
) []map[string]any {
	out := make([]map[string]any, len(entries))
	for i, e := range entries {
		out[i] = map[string]any{
			"ball1":        strconv.FormatInt(int64(e.Ball1), 10),
			"ball2":        strconv.FormatInt(int64(e.Ball2), 10),
			"ball3":        strconv.FormatInt(int64(e.Ball3), 10),
			"ball4":        strconv.FormatInt(int64(e.Ball4), 10),
			"ball5":        strconv.FormatInt(int64(e.Ball5), 10),
			"snaxBall":     strconv.FormatInt(int64(e.SnaxBall), 10),
			"ticketSerial": e.TicketSerial,
		}
	}
	return out
}
...
func CreateTradeTypedData(
	...
	case "clearSnaxpotPreference":
		message[API_WKS_nonce] = nonce.String()
		validated, err := ensureValidatedClearSnaxpotPreference(action)
		if err != nil {
			return apitypes.TypedData{}, fmt.Errorf("clearSnaxpotPreference payload: %w", err)
		}

		primaryType = "ClearSnaxpotPreference"
		message["scope"] = validated.Payload.Scope
	...
	case "saveSnaxpotTickets":
		message[API_WKS_nonce] = nonce.String()
		validated, err := ensureValidatedSaveSnaxpotTickets(action)
		if err != nil {
			return apitypes.TypedData{}, fmt.Errorf("saveSnaxpotTickets payload: %w", err)
		}

		primaryType = "SaveSnaxpotTickets"
		message["entries"] = toSaveSnaxpotTicketsTypedEntries(validated.Payload.Entries)
	...
	case "setSnaxpotPreference":
		message[API_WKS_nonce] = nonce.String()
		validated, err := ensureValidatedSetSnaxpotPreference(action)
		if err != nil {
			return apitypes.TypedData{}, fmt.Errorf("setSnaxpotPreference payload: %w", err)
		}

		primaryType = "SetSnaxpotPreference"
		message["snaxBall"] = strconv.FormatInt(int64(validated.Payload.SnaxBall), 10)
		message["scope"] = validated.Payload.Scope
	...
	case "voluntaryAutoExchange":
		message[API_WKS_nonce] = nonce.String()
		validated, err := ensureValidatedVoluntaryAutoExchange(action)
		if err != nil {
			return apitypes.TypedData{}, fmt.Errorf("voluntaryAutoExchange payload: %w", err)
		}

		primaryType = "VoluntaryAutoExchange"
		message[API_WKS_sourceAsset] = validated.SourceAsset
		message[API_WKS_targetUSDTAmount] = validated.TargetUSDTAmount
