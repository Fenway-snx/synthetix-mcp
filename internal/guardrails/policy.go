...
type Resolved struct {
	...
	hasMaxOrderNotional    bool
	hasMaxOrderQuantity    bool
	hasMaxPositionNotional bool
...
func (r *Resolved) HasMaxOrderNotional() bool {
	return r != nil && r.hasMaxOrderNotional
}
...
func (r *Resolved) HasMaxPositionNotional() bool {
	return r != nil && r.hasMaxPositionNotional
}
...
func (r *Resolved) IsSymbolAllowed(symbol string) bool {
	...
	if slices.Contains(r.AllowedSymbols, WildcardAll) {
		return true
	}
...
func resolveStandard(cfg *Config) (*Resolved, error) {
	...
	if slices.Contains(allowedOrderTypes, WildcardAll) {
		allowedOrderTypes = append([]string{}, supportedOrderTypes...)
	}
	...
	maxOrderNotional, hasMaxOrderNotional, err := parsePositiveDecimal(cfg.MaxOrderNotional, "maxOrderNotional")
	if err != nil {
		return nil, err
	}
	...
	return &Resolved{
		...
		RequestedPreset:        normalizePreset(cfg.Preset),
		hasMaxOrderNotional:    hasMaxOrderNotional,
		hasMaxOrderQuantity:    hasMaxOrderQuantity,
		hasMaxPositionNotional: hasMaxPositionNotional,
		hasMaxPositionQuantity: hasMaxPositionQuantity,
		writeEnabled:           true,
	}, nil
}
