...
// Configures the optional server-side EIP-712 signer.
// Only one key source may be set, and loopback binding is required by default.
...
type Config struct {
	...
	// Public Synthetix REST+WS endpoint used by the MCP service.
	...
	// Controls full-market response caching for single-market lookups.
	// Negative durations disable caching.
...
// Fails startup when broker signing lacks a key or safe bind posture.
...
// Accepts loopback hosts and rejects wildcard or deferred-resolution hosts.
// Operators running elsewhere can explicitly opt out.
