...
// canonical broker MCP tools. Those tools collapse the "discover subaccount →
// preview_auth_message → sign → authenticate → preview_trade_signature
// → sign → signed_place_order" choreography (which an LLM running in a
...
// in as X" hint surfaced by the broker tools.
