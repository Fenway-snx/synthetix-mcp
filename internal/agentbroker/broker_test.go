...
// gRPC resolver: it returns owned + delegated subaccount IDs without
...
// Locks the action-level allowlist gate. Adding any of these to
// allowedTradeActions silently widens the broker's blast radius;
// failing this test forces a deliberate review.
...
// regressions (e.g. re-introducing gRPC metadata) are deliberate.
