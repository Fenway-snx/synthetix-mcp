...
// REST-shaped stand-in returning owned and delegated subaccount IDs.
...
func TestSignAuthMessageProducesVerifiableSignature(t *testing.T) {
	...
	// Mirrors the live verification path by recovering the signer.
...
func TestSignTradeActionRoundTrip(t *testing.T) {
	...
	// Recompute expected typed data and verify it recovers the broker wallet.
...
// Locks delegated status placeholders after the REST migration.
