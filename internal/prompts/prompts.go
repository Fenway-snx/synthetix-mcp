...
// quickstart prompt body between the canonical self-hosted broker flow
// and the advanced wallet-holding flow so the rendered prompt can be executed verbatim
...
func quickstartWalletBody(symbol, side, qtyHint, subAccountID string) string {
	...
	return fmt.Sprintf(`You are onboarding to a Synthetix v4 off-chain MCP server and want to land your first authenticated trade: %s %s qty=%s on subaccount %s. The canonical path is the self-hosted broker using tools like place_order, but the self-hosted broker is DISABLED on this server. Continue only if the operator intentionally chose external wallet signing and you (the agent) hold a private key locally. If you do not, refuse the trade and tell the operator to enable the self-hosted broker (SNXMCP_AGENT_BROKER_ENABLED=true, see sample/node-scripts/scripts/onboard-agent-key.ts). NEVER ask the human user to paste an EIP-712 signature, hex digest, or private key into chat.
