...
func TestRegisterPromptsListAndRead(t *testing.T) {
	...
	for _, want := range []string{"BUY", "ETH-USDT", "2.5", "2300", "77"} {
	...
	for _, want := range []string{"SELL", "BTC-USDT", "0.01", "42", "preview_auth_message", "preview_trade_signature", "set_guardrails"} {
	...
	// Wallet-path body must NOT advertise quick_* — those are broker-only
	// and routing the agent through them when the broker is disabled
	// would surface NOT_FOUND from the tool registry.
	if strings.Contains(quickstartText.Text, "quick_place_order") {
		t.Fatalf("expected wallet-path quickstart NOT to mention quick_place_order, got %q", quickstartText.Text)
	}
}

// Locks the broker-on rendering path. Same prompt name + arguments,
// but the body must route exclusively through quick_* and must not
// instruct the agent to call authenticate / preview_* / place_order
// (which are duplicated work on a broker-enabled server and tempt the
...
func TestQuickstartPromptBrokerEnabled(t *testing.T) {
	...
	for _, want := range []string{"BUY", "BTC-USDT", "0.01", "quick_place_order", "quick_close_position", "quick_cancel_order"} {
		if !strings.Contains(text.Text, want) {
			t.Fatalf("expected broker quickstart prompt to contain %q, got %q", want, text.Text)
		}
	}
	for _, banned := range []string{"Call preview_auth_message", "Call preview_trade_signature", "Call authenticate", "Call place_order", "Call cancel_order", "Call modify_order", "Call close_position"} {
