...
func (m *Manager) Subscribe(sessionID string, requests []SubscribeRequest) ([]Subscription, []string, error) {
	...
	if added > 0 {
		metrics.SubscriptionEventsTotal("subscribed").Add(float64(added))
		metrics.ActiveSubscriptions().Add(float64(added))
	}
...
func (m *Manager) Unsubscribe(sessionID string, channels []string, symbol string) ([]Subscription, []Subscription, error) {
	...
	if n := len(removed); n > 0 {
		metrics.SubscriptionEventsTotal("unsubscribed").Add(float64(n))
		metrics.ActiveSubscriptions().Sub(float64(n))
	}
