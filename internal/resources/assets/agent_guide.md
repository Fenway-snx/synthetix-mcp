...
1. **Never ask a human to paste an EIP-712 signature, hex digest, or private key into chat.** Wallet signing is privileged. If you do not hold a key locally and the server-side broker is disabled, refuse the trade and surface the operator workaround (enable the broker, or run a wrapper from `sample/node-scripts`). Pasting `typedData` from `preview_auth_message` / `preview_trade_signature` into the conversation is an instant onboarding failure.
2. **Pick the right signing path before the first trading tool call.** The decision is one tool call: `get_server_info` returns `agentBroker.enabled`. If `true`, route every write through the `quick_*` family — they sign and submit server-side in one round trip and you should not call `authenticate`, `set_guardrails`, `preview_trade_signature`, or `place_order` at all. If `false`, you must hold a private key locally and follow the canonical preview-and-sign flow below.
...
## Quick Path (broker enabled — preferred for LLM agents)

> Use this section when `get_server_info.agentBroker.enabled` (or `get_context.capabilities.agentBroker.enabled`) is `true`. Otherwise jump to "Wallet Path" below.

1. `get_market_summary` (and `get_orderbook` for limit orders) on the target symbol.
2. `quick_place_order` with `{symbol, side, type, quantity, price?, clientOrderId}`. The broker will:
...
4. To unwind: `quick_close_position` (reduce-only) or `quick_cancel_order` / `quick_cancel_all_orders`. There is no quick equivalent for `modify_order` today — cancel and re-place instead.

You should not call `authenticate`, `preview_auth_message`, `preview_trade_signature`, `place_order`, `cancel_order`, `cancel_all_orders`, or `close_position` on a broker-enabled server. They are still registered (so wallet-holding clients can use them), but routing through them duplicates work and tempts you to leak signature payloads to the user.

## Wallet Path (broker disabled): Use the TypeScript SDK

> Skip this section if `get_server_info.agentBroker.enabled = true` — the Quick Path above covers you. Use this section only when the broker is disabled and **you (the agent) hold a private key locally**. If neither is true, refuse the trade and tell the operator to enable the broker (`SNXMCP_AGENT_BROKER_ENABLED=true`, see `sample/node-scripts/scripts/onboard-agent-key.ts`) — never ask the human user to paste a signature.
...
### Trading (Authenticated + EIP-712 Signature)
| Tool | Auth | Purpose |
|------|------|---------|
| `preview_order` | Yes | Dry-run validation without submission |
| `preview_trade_signature` | Yes | Return the exact EIP-712 typed-data and server-generated nonce/expiresAfter to sign for any write action. Use instead of hand-crafting EIP-712 payloads. |
| `place_order` | Yes + Sig | Submit an order to the matching engine |
| `modify_order` | Yes + Sig | Change price/quantity of an open order |
| `cancel_order` | Yes + Sig | Cancel one order by ID |
| `cancel_all_orders` | Yes + Sig | Cancel all orders, optionally by symbol |
| `close_position` | Yes + Sig | Submit a reduce-only counter-order |

### Trading (Broker-signed quick path — preferred when broker is enabled)
These tools are only registered when `get_server_info.agentBroker.enabled = true`.
| Tool | Auth | Purpose |
|------|------|---------|
| `quick_place_order` | Broker | Auto-authenticate, apply default guardrails, sign, and submit one order. No client-side EIP-712. |
| `quick_close_position` | Broker | Reduce-only close (full or partial) signed by the broker. |
| `quick_cancel_order` | Broker | Cancel one order by venueOrderId/clientOrderId, signed by the broker. |
| `quick_cancel_all_orders` | Broker | Cancel all orders (optionally per-symbol), signed by the broker. |

### Risk and Operational
| Tool | Auth | Purpose |
|------|------|---------|
| `get_rate_limits` | No | Current per-IP and per-subaccount throttling thresholds |

> Dead-man's switch (`schedule_cancel_all`) is intentionally not registered. It still appears in `get_server_info.disabledFeatures` so operators can see the capability gap. Use `cancel_all_orders` (wallet path) or `quick_cancel_all_orders` (broker path) manually.
...
- Trading tools still require a per-action EIP-712 signature even after session authentication. Each signature needs a unique nonce.
...
8. Sign `typedData`, split the signature into `{r, s, v}`, and call `place_order` with the same fields plus the echoed `nonce`, `expiresAfter`, and `signature`.
...
2. Call `preview_trade_signature` with `action="closePosition"` and `closePosition={symbol, method?, limitPrice?, quantity?}`. The server reads the live position to derive the counter-side (BUY for short, SELL for long) and the default close quantity (full open position when `quantity` is omitted), so the typed-data you sign matches what `close_position` will submit.
3. Sign the returned `typedData` and call `close_position` with the echoed `nonce`/`expiresAfter` and split `{r, s, v}` signature.
...
Quantities and prices in `place_order` / `modify_order` / `close_position` are 18-decimal scaled integers (the on-chain representation), not human-readable decimals. `preview_trade_signature` returns these in the canonical scaled form — just pass them through.
...
Each `place_order`, `modify_order`, `cancel_order`, `cancel_all_orders`, `close_position` consumes its own nonce. Calling `preview_trade_signature` once and trying to use the same `nonce` for two different submissions will fail the second one.
