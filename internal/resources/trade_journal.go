...
// Wires the authenticated account trade-journal resource.
// It aggregates the last 14 days of fills into markdown and optional cards.
...
// Decimal-safe local fill representation for journal aggregation.
...
// Converts upstream trade payloads into journal entries.
// The any input keeps tests independent of the full REST payload type.
...
// Aggregated journal shape consumed only by renderers.
...
// Renders the journal as markdown with an optional card block.
// Markdown gives chat clients headers and tables while terminals see plain text.
...
// Renders the compact card at the top of the journal.
// Daily PnL bars scale against the largest absolute day.
...
// Renders one row of daily PnL glyphs scaled to the largest day.
// Empty days render as dots to keep the time axis even.
...
// Parses REST time strings, falling back to millisecond epochs.
