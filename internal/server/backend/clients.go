...
// Clients bundles the public-API transport clients the mcp-service depends on.
type Clients struct {
	Public    *synthetix.Client
...
func NewClients(
	...
	publicClient, err := synthetix.NewClient(synthetix.Config{
	...
	if err != nil {
		return nil, fmt.Errorf("build public SDK client: %w", err)
	}
...
func (c *Clients) Close() error {
	...
	if c.Public != nil {
		if err := c.Public.Close(); err != nil && c.logger != nil {
			c.logger.Warn("close public SDK client failed", "error", err)
		}
	}
	if c.WSInfo != nil {
