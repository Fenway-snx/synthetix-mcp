...
func TestHealthRoutes(t *testing.T) {
	t.Run("ready", func(t *testing.T) {
		srv := newServer(snx_lib_logging_doubles.NewStubLogger(), testConfig(), &stubClients{}, nil, nil, nil, nil)
...
// TestUnknownPathsReturnJSONNotFoundForOAuthProbes pins the catch-all
// behaviour: every unmatched HTTP path -- not only /.well-known/* metadata
// endpoints, but also RFC 7591 Dynamic Client Registration (/register), the
// legacy RFC 6749 endpoints (/authorize, /token), and arbitrary other paths
// -- must return the JSON 404 envelope produced by handleWellKnownNotFound.
// Without this, OAuth-aware clients (notably Claude Code) crash trying to
// JSON.parse Go's default plain-text "404 page not found" body.
...
func TestInitializeRouteBindsStreamingSession(t *testing.T) {
	...
	srv := newServer(snx_lib_logging_doubles.NewStubLogger(), testConfig(), &stubClients{}, nil, streamingManager, nil, nil)
...
func TestClientIPFromRequestIgnoresForwardedHeaderFromUntrustedPeer(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/mcp", nil)
	req.RemoteAddr = "198.51.100.10:443"
	req.Header.Set("X-Forwarded-For", "203.0.113.25")

	got := clientIPFromRequest(req, []netip.Prefix{
...
func TestClientIPFromRequestUsesForwardedHeaderFromTrustedProxy(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/mcp", nil)
...
func TestClientIPFromRequestFallsBackWhenForwardedHeaderIsInvalid(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/mcp", nil)
	req.RemoteAddr = "10.1.2.3:443"
	...
	if got != "10.1.2.3" {
		t.Fatalf("expected proxy IP fallback, got %q", got)
	}
}

// TestClientIPFromRequestRejectsClientInjectedForwardedHeader verifies the
// attacker-controlled XFF case: when a client prepends their chosen IP to
// X-Forwarded-For, a well-behaved trusted proxy appends its observation of
// the actual peer. Right-to-left walking past the trusted hop must land on
// the real client IP, never the attacker-supplied value.
func TestClientIPFromRequestRejectsClientInjectedForwardedHeader(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/mcp", nil)
	req.RemoteAddr = "10.1.2.3:443"
	// Attacker tried to pass off 1.2.3.4 as the client IP; our trusted
	// proxy appended the real peer 203.0.113.25 at the end of the list.
	req.Header.Set("X-Forwarded-For", "1.2.3.4, 203.0.113.25")
...
// TestClientIPFromRequestWalksPastMultipleTrustedHops covers a chain of two
// trusted proxies. Each trusted hop must be skipped from the right until the
// first untrusted entry (the real client) is reached.
func TestClientIPFromRequestWalksPastMultipleTrustedHops(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/mcp", nil)
	req.RemoteAddr = "10.1.2.3:443"
	req.Header.Set("X-Forwarded-For", "203.0.113.25, 10.5.6.7, 10.1.2.3")
...
// TestClientIPFromRequestFallsBackWhenAllForwardedHopsAreTrusted makes sure
// that a header containing only trusted-CIDR values (e.g. an attacker
// spoofing 10.x values hoping to shift the walk) collapses back to the
// socket peer rather than returning a trusted-proxy IP.
func TestClientIPFromRequestFallsBackWhenAllForwardedHopsAreTrusted(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/mcp", nil)
	req.RemoteAddr = "10.1.2.3:443"
	req.Header.Set("X-Forwarded-For", "10.9.9.9, 10.1.2.3")

	got := clientIPFromRequest(req, []netip.Prefix{
		netip.MustParsePrefix("10.0.0.0/8"),
	})
	if got != "10.1.2.3" {
		t.Fatalf("expected socket peer fallback, got %q", got)
	}
}

// TestWellKnownOAuthProbeReturnsJSON404 verifies that OAuth-discovery probes
// (RFC 8414 / 9728 / OIDC) receive a JSON 404 that names the MCP tool-level
// auth model. This prevents confused clients (and client SDKs that mis-parse
// Go's default plain-text 404 as "Invalid OAuth error response") from getting
// stuck in an OAuth retry loop.
