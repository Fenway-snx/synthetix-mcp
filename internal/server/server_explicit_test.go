...
func TestLivenessCheckReturnsLiveStatus(t *testing.T) {
	srv := newServer(snx_lib_logging_doubles.NewStubLogger(), explicitServerConfig(), &stubClients{}, nil, nil, nil, nil)
...
func TestPublicHealthErrorRecognizesWrappedDeadlineExceeded(t *testing.T) {
	got := publicHealthError(errors.Join(errors.New("redis unavailable"), context.DeadlineExceeded))
...
func TestStartAndShutdownLifecycleSucceeds(t *testing.T) {
	...
	srv := newServer(snx_lib_logging_doubles.NewStubLogger(), explicitServerConfig(), clients, authManager, streamingManager, nil, nil)
...
func TestStartReturnsListenErrorForInvalidAddress(t *testing.T) {
	...
	srv := newServer(snx_lib_logging_doubles.NewStubLogger(), cfg, &stubClients{}, nil, nil, nil, nil)
