...
// This file is a fork of mcp.StreamableHTTPHandler from go-sdk v1.4.1.
//
// It adds capabilities the upstream handler does not yet expose:
//   - onSessionBound callback: notifies callers when a new session is
//     connected so streaming subscriptions can be wired up.
//   - boundServerTransport / boundConnection: wraps the transport to
//     intercept Close and clean up session-scoped resources.
//
// Upstream tracking: https://github.com/modelcontextprotocol/go-sdk/issues/479
// (proposes OnConnectionClose / OnSessionClose in StreamableHTTPOptions).
// When that lands, evaluate collapsing this fork back to the SDK handler.
...
// createSession allocates a session ID (if needed), connects it to the
// MCP server, records it in the session map, and invokes onSessionBound
// so streaming subscriptions can be wired up. On error it writes an HTTP
// response and returns a non-nil error so the caller can return early.
func (h *streamableSessionHandler) createSession(
	...
	// Use a session-scoped context so the JSONRPC2 connection (and its
	// EventStore.SessionClosed callback) outlives the initialize POST request.
	// req.Context() is cancelled when the POST handler returns, which would
	// prematurely tear down streaming state. cancelSession is called from
	// onClose when the session is truly done.
