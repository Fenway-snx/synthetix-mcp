...
func New(
	...
	// Build signing support before registering the write tool surface.
	var (
	...
	// Push position-closed events over the bound MCP session connection.
	// The service owns its goroutine and is stopped during shutdown.
...
func newServer(
	...
	// Keep unmatched OAuth probes on the JSON 404 path.
	// Some clients parse every non-2xx response body as JSON.
...
// Returns a JSON 404 for OAuth discovery probes.
// The body points clients toward the MCP tool-level auth flow.
...
// Renders the MCP Instructions prompt with signing and routing rules.
...
// Adapts broker status to the tools package snapshot shape.
type brokerStatusAdapter struct {
	broker *agentbroker.Broker
}

// Forwards broker read-signing through the narrow tools interface.
...
// Bridges risk hydration to broker-signed REST trade reads.
...
// Builds the in-process signer using the same EIP-712 domain as auth.
