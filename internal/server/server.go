...
func New(
	...
	eventStore := streaming.NewEventStore(func(sessionID string) {
		...
		metrics.SessionEventsTotal("expired").Inc()
		publicSessions.Forget(sessionID)
		// Use DeleteIfExists as the atomic gate: it returns true only
		// for the caller that removed the key, preventing double
		// decrements on concurrent revocation.
		state, err := sessionStore.Get(context.Background(), sessionID)
		if err != nil || state == nil || state.AuthMode != session.AuthModeAuthenticated {
			return
		}
		if deleted, delErr := sessionStore.DeleteIfExists(context.Background(), sessionID); delErr == nil && deleted {
			metrics.ActiveSessions().Dec()
		}
	})
	rateLimiter, err := tools.NewRateLimiter(context.Background(), logger, cfg)
	if err != nil {
		return nil, errors.Join(fmt.Errorf("create rate limiter: %w", err), streamingManager.Close(), authManager.Close(), clients.Close())
	}

	srv := newServer(logger, cfg, clients, authManager, streamingManager, eventStore, rateLimiter)
	...
	// Broker and REST-signed shim are built before trade-tool
	// registration so all consumers share the same TradeReadClient.
	var (
	...
	tools.RegisterTradingTools(srv.mcpServer, toolDeps, restMarketReaderAdapter{rest: clients.RESTInfo}, tradeReads, riskSnapshotManager, authManager)
	tools.RegisterSignaturePreviewTools(srv.mcpServer, toolDeps, authManager, tradeReads)
	if broker != nil {
		tools.RegisterQuickTools(
			srv.mcpServer, toolDeps, broker, authManager, tradeReads,
			riskSnapshotManager,
		)
		logger.Info(
			"agent broker quick_* tools registered",
	...
	tools.RegisterRiskTools(srv.mcpServer, toolDeps)
...
func newServer(
	...
	trustedProxyPrefixes := trustedProxyPrefixesFromConfig(cfg)
	...
	mcpHTTPHandler := wrapMCPHandlerWithRateLimit(logger, mcpHandler, limiter, trustedProxyPrefixes)
	...
	mux.Handle("/mcp", http.MaxBytesHandler(mcpHTTPHandler, cfg.MaxRequestBodyBytes))
...
func clientIPFromRequest(r *http.Request, trustedProxyPrefixes []netip.Prefix) string {
	if r == nil {
		return ""
	}
	...
	if isTrustedProxy(remoteIP, trustedProxyPrefixes) {
		if forwardedIP, ok := forwardedClientIP(r.Header.Get("X-Forwarded-For"), trustedProxyPrefixes); ok {
...
// IP that is NOT in the trusted-proxy set. This is the standard defense
// against client-controlled XFF spoofing: each trusted proxy appends the peer
// it observed, so the genuine client is the rightmost non-trusted entry. Any
// values to the LEFT of that point were either supplied by a trusted proxy
// (and thus correct) or injected by the client (and thus untrustworthy) --
// either way we stop at the first untrusted hop.
func forwardedClientIP(raw string, trustedProxyPrefixes []netip.Prefix) (netip.Addr, bool) {
	...
	for i := len(parts) - 1; i >= 0; i-- {
		...
		addr = addr.Unmap()
		if !isTrustedProxy(addr, trustedProxyPrefixes) {
			return addr, true
		}
	}
...
func isTrustedProxy(addr netip.Addr, trustedProxyPrefixes []netip.Prefix) bool {
...
func remoteIPFromRequest(r *http.Request) (netip.Addr, bool) {
	...
	addr, err := netip.ParseAddr(strings.TrimSpace(r.RemoteAddr))
	if err != nil {
		return netip.Addr{}, false
	}
	return addr.Unmap(), true
...
// prefer quick_* when the broker is enabled (only advertised when
...
func buildServerInstructions(brokerEnabled bool) string {
	const base = "Synthetix MCP service is online. Start with get_context " +
		"or read system://agent-guide to orient to the current session, " +
		"markets, and trading workflows."
	...
	if brokerEnabled {
		return base + noPasteRule + " The agent broker is enabled on this " +
			"server: prefer quick_place_order, quick_close_position, " +
			"quick_cancel_order, and quick_cancel_all_orders, which sign " +
			"and submit in one call with no client-side cryptography."
	}
	return base + noPasteRule + " The agent broker is disabled on this " +
		"server. If you do not hold a private key locally, refuse the " +
		"trade and ask the operator to enable the broker " +
		"(SNXMCP_AGENT_BROKER_ENABLED=true plus a private key) or run an " +
		"SDK-backed wrapper (sample/node-scripts) instead of pasting " +
		"signatures into chat."
}
...
// would cause every quick_* call to fail with INVALID_SIGNATURE.
...
func trustedProxyPrefixesFromConfig(cfg *config.Config) []netip.Prefix {
	if cfg == nil || len(cfg.TrustedProxyCIDRs) == 0 {
		return []netip.Prefix{}
	}
