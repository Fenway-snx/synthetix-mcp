// Package sdkparity contains tests that verify the public Synthetix
// Go SDK (services/mcp/sdk/eip712 + services/mcp/sdk/signer) produces
// EIP-712 digests that are byte-for-byte identical to the canonical
// monorepo implementation (lib/auth.CreateTradeTypedData and
// lib/auth.GetEIP712MessageHash).
//
// Living outside the SDK tree is deliberate: these tests import
// lib/auth and lib/api/validation, which the SDK itself never does.
// When the SDK is extracted into its own repo, this file stays in
// the monorepo and continues guarding wire-format parity until the
// monorepo migrates to consuming the SDK module.
...
// digestsEqual asserts that two EIP-712 typed-data structures hash
// to the identical 32-byte digest. The SDK's TypedData is an alias
// for apitypes.TypedData so canonical (lib/auth) and SDK builders
// can share the same hasher.
