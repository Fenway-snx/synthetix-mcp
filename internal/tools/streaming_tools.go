...
func RegisterStreamingTools(
	...
	mcp.AddTool(server, &mcp.Tool{
	...
	}, func(ctx context.Context, req *mcp.CallToolRequest, input subscribeInput) (*mcp.CallToolResult, subscribeOutput, error) {
		start := snx_lib_utils_time.Now()
		if len(input.Subscriptions) == 0 {
			recordToolOutcome("subscribe", "error", start)
		...
		if err := MaybeRateLimitOperation(ctx, deps.Limiter, state, "subscribe", len(input.Subscriptions)); err != nil {
			recordToolOutcome("subscribe", "rate_limited", start)
			return toolErrorResponse[subscribeOutput](err)
		}
		...
		for _, sub := range input.Subscriptions {
			...
			// with the NATS consumer: this endpoint has no source for
		...
		recordToolOutcome("subscribe", "ok", start)
	...
	mcp.AddTool(server, &mcp.Tool{
	...
	}, func(ctx context.Context, req *mcp.CallToolRequest, input unsubscribeInput) (*mcp.CallToolResult, unsubscribeOutput, error) {
		start := snx_lib_utils_time.Now()
		if len(input.Channels) == 0 {
			recordToolOutcome("unsubscribe", "error", start)
		...
		if err := MaybeRateLimitOperation(ctx, deps.Limiter, state, "unsubscribe", len(input.Channels)); err != nil {
			recordToolOutcome("unsubscribe", "rate_limited", start)
			return toolErrorResponse[unsubscribeOutput](err)
		}
		if err := touchSession(ctx, deps.Store, sessionID, deps.Cfg.SessionTTL); err != nil {
			recordToolOutcome("unsubscribe", "error", start)
		...
		recordToolOutcome("unsubscribe", "ok", start)
