...
// TestClassifyToolErrorRoundTripPreservesBehavior pins every branch of
// classifyToolError against an explicit expected-output fixture so future
// refactors (e.g. converting the switch to a data table) stay safe. Each
// case declares its input and the exact (code, message, len(remediation))
// triple we return to agents; mismatches fail fast with a labelled message.
func TestClassifyToolErrorRoundTripPreservesBehavior(t *testing.T) {
	...
	cases := []caseT{
		...
		},
		{
			...
			want: expected{"RATE_LIMITED", "Request rate limit exceeded.", 1},
		},
		{
			name: "phrase_timeout",
