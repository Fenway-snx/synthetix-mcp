...
func TestRegisterAccountToolsRegistersExpectedTools(t *testing.T) {
	...
	RegisterAccountTools(server, testAccountToolDeps(&fakeSessionStore{sessions: map[string]*session.State{}}, nil, nil), nil)
...
func TestGetAccountSummaryRequiresAuthenticatedSession(t *testing.T) {
	...
	RegisterAccountTools(server, testAccountToolDeps(store, nil, nil), nil)
...
func testAccountToolDeps(store session.Store, verifier SessionAccessVerifier, limiter ToolRateLimiter) *ToolDeps {
	return &ToolDeps{
...
// the early-return path that used to be masked by the gRPC subaccount
...
func TestGetAccountSummaryClearsStaleSessionWhenVerifierRejects(t *testing.T) {
	...
	RegisterAccountTools(
		server,
		testAccountToolDeps(store, &fakeSessionVerifier{err: errors.New("wallet is no longer authorized")}, nil),
		nil,
	)
