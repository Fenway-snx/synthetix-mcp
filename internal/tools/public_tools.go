...
// Mirrors broker capability and posture for server discovery.
// Optional fields stay absent until the first broker write binds state.
...
func RegisterPublicTools(server *mcp.Server, deps *ToolDeps) {
	...
	addPublicTool(server, deps, &mcp.Tool{
		...
		// REST only carries aggressor side, so keep fillType empty.
	...
	addPublicTool(server, deps, &mcp.Tool{
		...
		// Always emit a slice so JSON consumers can iterate directly.
		// Delegated owner addresses are omitted because REST does not return them.
...
// Parses REST trade IDs into the retained uint64 output shape.
...
// Parses REST decimal subaccount IDs into int64.
// Non-numeric values are dropped by the caller.
func strconvParseInt(raw string) (int64, error) {
	return strconv.ParseInt(strings.TrimSpace(raw), 10, 64)
}

// Projects broker config and live posture into server discovery.
// A populated struct makes enabled:false unambiguous.
...
// Duplicated to keep broker internals out of this package's imports.
const brokerSubaccountSourceOwned = "owned"

// Returns the REST info client or an agent-facing configuration error.
