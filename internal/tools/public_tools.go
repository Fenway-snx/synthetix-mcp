...
// fields stay absent until the first quick_* call so the not-yet-
...
func RegisterPublicTools(server *mcp.Server, deps *ToolDeps) {
	addRateLimitedTool(server, deps, &mcp.Tool{
		Name:        "ping",
	...
	addRateLimitedTool(server, deps, &mcp.Tool{
		...
		Description: "Return server identity, supported auth modes, enabled streaming channels, agent-broker availability, and disabled features. Use this after ping to discover available capabilities before trading. agentBroker.enabled=true means the server signs EIP-712 internally and you should call quick_* tools rather than authenticate / preview_trade_signature / place_order.",
...
// Malformed IDs land as zero, matching the zero value for gRPC-
// sourced responses that skipped the field.
...
func buildServerInfoAgentBroker(deps *ToolDeps) serverInfoAgentBroker {
	...
	out := serverInfoAgentBroker{
		...
		Enabled:       true,
		Note: "Broker enabled. Call quick_place_order / " +
			"quick_close_position / quick_cancel_order / " +
			"quick_cancel_all_orders for one-shot sign+submit. " +
			"Do not call authenticate, preview_trade_signature, or " +
			"place_order unless you also hold a private key locally.",
		QuickTools: []string{
			"quick_place_order",
...
// Defensive: api_base_url is required at Load() and in Docker compose,
// but if somehow the clients are absent we refuse cleanly rather than
// NPE.
