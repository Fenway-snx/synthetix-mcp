...
func RegisterAccountTools(
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		...
		// Apply filters locally to avoid a second REST shim variant.
		// Upstream default cap bounds the local filtering cost.
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		...
		// Preserve offset pagination locally; REST returns a desc-sorted slice.
		limit := input.Limit
		offset := input.Offset
...
// Projects collateral rows onto the public output shape.
// Existing field names are kept for the Phase-0 contract.
...
// Projects the cross-margin summary onto the Phase-0 field names.
...
// Narrows REST order IDs to the public output shape.
// Non-numeric venue IDs degrade to zero rather than failing the whole tool.
...
// Projects position rows onto the Phase-0 output shape.
// Omitted REST fields remain empty; response metadata flags the backend.
...
// Projects open-order rows onto the Phase-0 output shape.
// Remaining quantity is derived from quantity minus filled quantity.
// Legacy flat ID fields are left empty or derived from nested IDs.
...
// Coerces decimal wire IDs into uint64, returning zero on parse failure.
// Mapping happens inside one-shot tool responses, so errors cannot propagate.
...
// Coerces decimal wire IDs into int64, returning zero on parse failure.
...
// Renders Unix milliseconds as RFC-3339 UTC.
// Empty output omits optional timestamp fields.
...
// Derives remaining quantity from REST quantity and filled quantity.
// Falls back to raw quantity when values are empty or malformed.
...
// Assembles REST order-history params from tool input.
// Unexposed fields stay unset so service defaults apply.
func buildOrderHistoryParams(input getOrderHistoryInput) map[string]any {
	...
	// Pass through limit to minimize payload; offset is applied locally.
	if input.Limit > 0 {
...
// Assembles REST trade-history params from tool input.
// The endpoint supports native offset/limit pagination.
...
// Assembles REST funding-payment params from supported filters.
// The endpoint ignores pagination offset, so it is omitted.
...
// Applies offset/limit locally to desc-sorted order history.
// This preserves the public pagination contract.
...
// Projects order-history rows onto the Phase-0 output shape.
// Per-item fields missing from REST remain empty.
...
// Projects trade rows onto the Phase-0 output shape.
// The maker boolean becomes the existing fill-type label.
...
// Reconstructs the Phase-0 fill-type label from REST's maker flag.
...
// Projects funding-payment rows onto the Phase-0 output shape.
// Canonical time fields are used instead of deprecated aliases.
...
// Projects funding summary fields onto the Phase-0 output shape.
...
// Projects performance points onto the Phase-0 output shape.
