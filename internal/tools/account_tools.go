...
// OrderAction — both gRPC-only) are left as empty strings; the
...
// flat venue-id fields from gRPC are left unset or derived from the
...
// `fillType` string (gRPC-era enum); map it back to the Phase-0
...
// the REST `maker` boolean. The gRPC surface exposed the full
// enum (MAKER / TAKER / LIQUIDATION / …) but the REST wire only
...
// Phase-0 fundingSummaryOutput shape. Straight rename — the REST
// wire and the gRPC wire agree on field names for this block.
