...
// place_order/quick_place_order phase + followUp fields. The original
...
func TestNextStepsForAuthenticatedSession(t *testing.T) {
	...
	t.Run("standard_preset_recommends_quick_or_preview_path", func(t *testing.T) {
		state := &session.State{
			AuthMode:      session.AuthModeAuthenticated,
			SubAccountID:  1,
			WalletAddress: "0x",
		...
		if !containsSubstring(steps, "place_order") && !containsSubstring(steps, "quick_place_order") {
...
// get_context tells the agent "use quick_*" or "sign locally". The
// regressions we want to catch are subtle: forgetting to populate
// quickTools when the broker is on, or forgetting the "never paste
...
func TestContextCapabilitiesFromFlagsBrokerEnabled(t *testing.T) {
	...
	if len(caps.AgentBroker.QuickTools) == 0 {
		t.Fatalf("expected QuickTools to be populated when broker enabled")
	}
	wantQuick := map[string]bool{
		"quick_place_order":       true,
		"quick_close_position":    true,
		"quick_cancel_order":      true,
		"quick_cancel_all_orders": true,
	}
	for _, tool := range caps.AgentBroker.QuickTools {
		if !wantQuick[tool] {
			t.Fatalf("unexpected quick tool advertised: %q", tool)
		}
		delete(wantQuick, tool)
	}
	if len(wantQuick) != 0 {
		t.Fatalf("expected all four quick tools advertised, missing %v", wantQuick)
	}
	...
	if !containsSubstring(caps.RecommendedFlow, "quick_place_order") {
		t.Fatalf("expected recommendedFlow to point at quick_place_order, got %v", caps.RecommendedFlow)
	}
}
...
func TestContextCapabilitiesFromFlagsBrokerDisabled(t *testing.T) {
	...
	if len(caps.AgentBroker.QuickTools) != 0 {
		t.Fatalf("expected no quickTools when broker disabled, got %v", caps.AgentBroker.QuickTools)
	}
	...
	for _, mustMention := range []string{"preview_auth_message", "authenticate", "set_guardrails", "place_order"} {
...
func TestBuildServerInfoAgentBrokerEnabled(t *testing.T) {
	...
	if len(info.QuickTools) != 4 {
		t.Fatalf("expected four quickTools, got %v", info.QuickTools)
	}
	if !strings.Contains(info.Note, "quick_place_order") {
		t.Fatalf("expected note to call out quick_place_order, got %q", info.Note)
	}
}
...
func TestBuildServerInfoAgentBrokerDisabled(t *testing.T) {
	...
	if len(info.QuickTools) != 0 {
		t.Fatalf("expected empty quickTools when disabled, got %v", info.QuickTools)
	}
