...
// Builds the validated action payload for signature preview.
// Close previews resolve live position data unless side and quantity are explicit.
func buildTradePreviewPayload(
	...
	case "closePosition":
		...
		// Skip the REST pre-flight when side and quantity are explicit.
		...
		return validated, snx_lib_api_types.RequestAction("placeOrders"), nil
	case "updateLeverage":
		if input.UpdateLeverage == nil {
			return nil, "", fmt.Errorf("updateLeverage payload is required for action=updateLeverage")
		}
		validated, _, err := buildUpdateLeveragePayload(input.UpdateLeverage.Symbol, input.UpdateLeverage.Leverage)
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("updateLeverage"), nil
	case "withdrawCollateral":
		if input.WithdrawCollateral == nil {
			return nil, "", fmt.Errorf("withdrawCollateral payload is required for action=withdrawCollateral")
		}
		validated, _, err := buildWithdrawCollateralPayload(input.WithdrawCollateral.Symbol, input.WithdrawCollateral.Amount, input.WithdrawCollateral.Destination)
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("withdrawCollateral"), nil
	case "transferCollateral":
		if input.TransferCollateral == nil {
			return nil, "", fmt.Errorf("transferCollateral payload is required for action=transferCollateral")
		}
		validated, _, err := buildTransferCollateralPayload(input.TransferCollateral.ToSubAccountID.Int64(), input.TransferCollateral.Symbol, input.TransferCollateral.Amount)
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("transferCollateral"), nil
	case "scheduleCancel":
		if input.ScheduleCancel == nil {
			return nil, "", fmt.Errorf("scheduleCancel payload is required for action=scheduleCancel")
		}
		validated, _, err := buildScheduleCancelPayload(input.ScheduleCancel.TimeoutSeconds)
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("scheduleCancel"), nil
	case "addDelegatedSigner":
		if input.AddDelegatedSigner == nil {
			return nil, "", fmt.Errorf("addDelegatedSigner payload is required for action=addDelegatedSigner")
		}
		validated, _, err := buildAddDelegatedSignerPayload(input.AddDelegatedSigner.DelegateAddress, input.AddDelegatedSigner.Permissions, input.AddDelegatedSigner.ExpiresAt)
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("addDelegatedSigner"), nil
	case "removeDelegatedSigner":
		if input.RemoveDelegatedSigner == nil {
			return nil, "", fmt.Errorf("removeDelegatedSigner payload is required for action=removeDelegatedSigner")
		}
		validated, _, err := buildRemoveDelegatedSignerPayload(input.RemoveDelegatedSigner.DelegateAddress)
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("removeDelegatedSigner"), nil
	case "removeAllDelegatedSigners":
		if input.RemoveAllDelegatedSigners == nil {
			return nil, "", fmt.Errorf("removeAllDelegatedSigners payload is required for action=removeAllDelegatedSigners")
		}
		validated, _, err := buildRemoveAllDelegatedSignersPayload()
		if err != nil {
			return nil, "", err
		}
		return validated, snx_lib_api_types.RequestAction("removeAllDelegatedSigners"), nil
	default:
