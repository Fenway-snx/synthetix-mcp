...
// renderMarketSummaryCard renders a compact 80-column dashboard for a
// single market: mark / index / last / best bid / best ask, 24h change
// and volume, open interest, and a visual funding-rate bar. Agents use
// this as a pre-trade sanity check; humans glance at it to sense
// whether the market is hot, flat, or funding-loaded.
...
// renderListMarketsCard renders a top-N leaderboard of markets sorted
// by 24h quote volume. Each row shows symbol, mark, 24h change with
// sign glyph, and a compact volume bar so the trader can see where
// liquidity is concentrated at a glance.
...
// fundingBar renders a centered ±cap gauge. Empty cells are dots,
// the centerline is a `│`, and the bar fills from center toward
// the edge in the direction of the rate.
...
// sparkline24h renders a minimal "prev → last" segment. With only
// two real data points the upstream REST surface gives us, a full
// sparkline would be noise; this shows direction + magnitude.
