...
type closePositionInput struct {
	...
	Side         string              `json:"side,omitempty" jsonschema:"Optional side of the existing position: 'long' or 'short'. When provided, close_position skips the getPositions pre-flight and uses the caller-supplied side + quantity directly. Required when the agent broker is disabled (no broker-signed positions read is available). When omitted, close_position fetches positions via the broker-signed REST read and infers side."`
...
func RegisterTradingTools(
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		...
		func(ctx context.Context, tc ToolContext, input previewOrderInput) (*mcp.CallToolResult, previewOrderOutput, error) {
			...
			validationErrors = append(validationErrors, guardrailPreviewErrors(ctx, tc.SessionID, tc.State, snapshotManager, normalized)...)
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		Name:        "place_order",
		Description: "Submit one order to the Synthetix matching engine. Requires an authenticated session and a per-action EIP-712 signature with nonce. Supports LIMIT (GTC/IOC), MARKET, STOP, STOP_MARKET, TAKE_PROFIT, and TAKE_PROFIT_MARKET order types. Recommended flow: (1) preview_order to validate order shape, (2) preview_trade_signature with action='placeOrders' to obtain the typed-data object plus a server-generated nonce/expiresAfter, (3) sign the typedData with the authenticated wallet, (4) call place_order with the same order fields, the echoed nonce and expiresAfter, and the split {r, s, v} signature. AGENT POLICY: only call this if you (the agent) hold the signing key locally. If get_server_info.agentBroker.enabled=true, call quick_place_order instead — it auto-authenticates, signs internally, and submits in one round trip with no user interaction.",
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		Name:        "modify_order",
		Description: "Modify the price, quantity, or trigger price of one open order. Identify the order by venueOrderId or clientOrderId (not both). Requires a per-action EIP-712 signature. The order retains its queue priority only if price is unchanged. Use preview_trade_signature with action='modifyOrder' to obtain the exact typed-data to sign. AGENT POLICY: only call this if you (the agent) hold the signing key locally; never ask a human to paste signatures into chat. (modify_order has no quick_* equivalent today — cancel via quick_cancel_order and re-place via quick_place_order if the broker is enabled.)",
		...
		func(ctx context.Context, tc ToolContext, input modifyOrderInput) (*mcp.CallToolResult, modifyOrderOutput, error) {
			...
			existingOrder, err := enforceModifyOrderGuardrails(ctx, tc.SessionID, tc.State, snapshotManager, input, venueOrderID, clientOrderID)
			if err != nil {
				return toolErrorResponse[modifyOrderOutput](err)
			}
			if err := authenticator.ValidateTradeAction(
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		Name:        "cancel_order",
		Description: "Cancel one open order by venueOrderId or clientOrderId (not both). Requires a per-action EIP-712 signature. Returns the cancelled order details or an error if the order was already filled or does not exist. Use preview_trade_signature with action='cancelOrders' to obtain the typed-data to sign. AGENT POLICY: prefer quick_cancel_order when get_server_info.agentBroker.enabled=true; do not paste signature payloads into chat.",
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		...
		Description: "Cancel all open orders for the authenticated subaccount. Optionally filter by symbol to cancel only orders on one market. Requires a per-action EIP-712 signature. Always call get_open_orders first to confirm which orders will be affected. Use preview_trade_signature with action='cancelAllOrders' to obtain the typed-data to sign. AGENT POLICY: prefer quick_cancel_all_orders when get_server_info.agentBroker.enabled=true; do not paste signature payloads into chat.",
	...
	addAuthenticatedTool(server, deps, &mcp.Tool{
		Name:        "close_position",
		Description: "Close an open position by submitting a reduce-only counter-order. Defaults to a market order for full close; specify quantity for partial close or method:limit with limitPrice for limit close. Requires a per-action EIP-712 signature. When side is omitted, close_position fetches positions via the broker-signed REST read to infer side and (if quantity is omitted) full-position size. When broker is disabled, pass explicit side ('long' or 'short') and quantity to skip the pre-flight. Use preview_trade_signature with action='closePosition' to obtain the typed-data to sign. AGENT POLICY: prefer quick_close_position when get_server_info.agentBroker.enabled=true; do not paste signature payloads into chat.",
...
func normalizePlaceOrder(input previewOrderInput) (snx_lib_api_json.PlaceOrderRequest, normalizedOrderOutput, error) {
	...
	if side != "BUY" && side != "SELL" {
		return snx_lib_api_json.PlaceOrderRequest{}, normalizedOrderOutput{}, fmt.Errorf("side must be BUY or SELL")
	}
	...
	return snx_lib_api_json.PlaceOrderRequest{
		Symbol:          snx_lib_api_json.Symbol(input.Symbol),
...
func mapMCPOrderType(orderType string, tif string) (string, bool, string, string, error) {
	...
	case "LIMIT":
		if tif == "" || tif == "GTC" {
			return "limitGtc", false, "LIMIT", "GTC", nil
		}
		if tif == "IOC" {
			return "limitIoc", false, "LIMIT", "IOC", nil
		}
...
func resolveClosablePositionOrExplicit(
	...
	if side != "" {
		if side != "long" && side != "short" {
			return "", decimal.Zero, fmt.Errorf("side must be 'long' or 'short'")
		}
...
func classifyPlaceOrderPhase(status string, isSuccess bool, errorCode string) (string, []string) {
	...
	default:
		return OrderPhasePendingConfirmation, []string{
			"The trading service ack arrived before the matching engine applied the order.",
