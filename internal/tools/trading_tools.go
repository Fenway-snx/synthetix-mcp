...
// Buckets upstream order statuses into accepted, rejected, or pending.
// UNKNOWN/empty means callers should poll, not retry, to avoid double-fills.
const (
...
// Uses explicit side and quantity when supplied, otherwise reads positions.
// The explicit path supports wallet-only deployments without broker reads.
...
// Reads positions and resolves the long or short exposure for a symbol.
// Wallet-only sessions must pass explicit side and quantity instead.
...
// Combines status, success, and error code into phase plus follow-up.
// UNKNOWN remains pending to prevent unsafe retries during engine races.
