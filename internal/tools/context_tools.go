...
// Summarizes server capabilities so agents can choose the right flow.
...
// Registers context tooling and optional authenticated account summaries.
func RegisterContextTools(
	...
	addPublicTool(server, deps, &mcp.Tool{
		...
		// Leave account empty when broker-read support is unavailable.
		var (
...
func contextCapabilitiesFromFlags(
	...
	caps := contextCapabilities{
		AgentBroker: contextAgentBroker{
			...
			// Keep this slice non-nil so JSON consumers can iterate safely.
			BrokerTools: []string{},
		},
	}
	if brokerEnabled {
