...
type contextAgentBroker struct {
	...
	QuickTools    []string `json:"quickTools"`
}
...
func contextCapabilitiesFromFlags(
	...
	caps := contextCapabilities{
		AgentBroker: contextAgentBroker{
			...
			// QuickTools is intentionally always a (possibly empty)
	...
	if brokerEnabled {
		...
		caps.AgentBroker.QuickTools = []string{
			"quick_place_order",
		...
		caps.AgentBroker.Note = "Broker holds the trading key " +
			"server-side. Call quick_* tools for one-shot " +
			"sign+submit; you do not need to call authenticate, " +
			"set_guardrails, preview_trade_signature, or place_order."
		...
		caps.RecommendedFlow = []string{
			...
			"2. Call quick_place_order with {symbol, side, type, quantity, price?, clientOrderId}. " +
			...
			"3. Inspect the returned phase and followUp; call get_open_orders / get_order_history with the returned clientOrderId to confirm the final state.",
			"4. To unwind: call quick_close_position (reduce-only) or quick_cancel_order — never simulate signing in chat.",
		}
	} else {
		...
		caps.RecommendedFlow = []string{
			...
			"4. Call set_guardrails with preset='standard' (sessions default to read_only).",
			"5. For each trade: preview_trade_signature → sign locally → place_order (or modify_order / cancel_order / cancel_all_orders / close_position) with the echoed nonce + expiresAfter + split signature.",
			"6. If you have no local signer, do NOT prompt the user — refuse the trade and explain that the operator must enable the broker or run sample/node-scripts.",
		}
	}
	return caps
}
