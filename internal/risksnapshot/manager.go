...
// Emitted when a fresh snapshot changes position quantity meaningfully.
// Closed, opened, flipped, and adjusted states are surfaced to observers.
...
// Enumerates position quantity changes detectable from snapshots alone.
...
// Invoked synchronously after locks are released.
// Observers should use another goroutine for follow-up manager calls.
...
// Registers an observer for position changes after fresh hydration.
// Calls run synchronously after the manager releases its lock.
...
// Compares snapshot quantities and emits meaningful position transitions.
// Same-side increases are ignored; trims still emit adjusted transitions.
